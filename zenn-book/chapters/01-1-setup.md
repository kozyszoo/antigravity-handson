---
title: "Step 1-1: 環境セットアップ"
---

## 目標
Antigravity を起動して、ワークスペースの基本設定まで済ませましょう。
...
### 1. Antigravity CLI のインストール確認
- Antigravity CLIがインストールされている
- Google アカウントでログイン済み
...
2. **ブラウザ拡張機能**（任意・後で設定してもOK）:
   - Antigravity CLI の設定画面（Agent パネル右上の `...` → `MCP Store`）から Browser 連携ツールを追加できます
   - 今すぐ必要ではないので、Step 3-1（MCP接続）で改めて説明します

### 3. ワークスペースの作成

Antigravity CLI を起動し、メニューから以下の操作を行ってください。
...
| チャットが応答しない | ネットワーク接続を確認 / Antigravity CLI を再起動 |
